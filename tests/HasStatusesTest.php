...
class HasStatusesTest extends TestCase
{
    ...
    public function it_can_return_the_latest_status()
    {
        ...
        $this->assertEquals('status 1', $this->testModel->latestStatus('status 1', 'status 3'));

        $this->assertEquals('status 1', $this->testModel->latestStatus(['status 1', 'status 3']));

        $this->assertEquals('status 2', $this->testModel->latestStatus('status 1', 'status 2', 'status 3'));
    ...
    {
        $this->app['config']->set('model-status.status_model', AlternativeStatusModel::class);
    ...
    public function it_can_use_a_custom_name_for_the_relationship_id_column()
    {
        ...
        $model = TestModel::create(['name' => 'model']);
